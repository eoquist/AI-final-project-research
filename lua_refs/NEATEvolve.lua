...
GLOBAL VARIABLES:
	...
	rightmost --> initializeRun

	pool --> initializePool
	
	controller --> clearJoypad

	accessing savestate. --> initializeRun 

	-- look into how > writeFile("backup." .. pool.generation .. "." .. forms.gettext(saveLoadFile)) < works
...
function getPositions()
	...
	-- layer that contains the solid ground and most of the game's obstacles and enemies
...
function getTile(dx, dy)
	--[[ In SMW, the tiles are the 16x16 pixel squares that making up the game's background and objects. 
		Each tile is assigned a unique value, which corresponds to a specific graphic or gameplay element. 
		The game uses a tile-based rendering system, where the graphics are drawn on the screen by tiling 
		small images together to create the overall scene. ]]
    x = math.floor((marioX + dx + 8) / 16) -- + 8 pixels to account for centering Mario
    y = math.floor((marioY + dy) / 16)

	return memory.readbyte(0x1C800 + math.floor(x / 16) * 0x1B0 + y * 16 + x % 16)
	-- 0x1C800 is the starting address for the level data
	-- 
end

function getSprites()
    local sprites = {}
    for slot = 0, 11 do -- game can only load up to 12 unique sprites into the OAM at once
        local status = memory.readbyte(0x14C8 + slot)
        if status ~= 0 then 
			-- there is a sprite present in that slot
            local spritex = memory.readbyte(0xE4 + slot) + memory.readbyte(0x14E0 + slot) * 256 -- low byte + high byte
            local spritey = memory.readbyte(0xD8 + slot) + memory.readbyte(0x14D4 + slot) * 256 -- low byte + high byte
			-- (*256) ==> shifting the high byte left by 8 bits

            sprites[#sprites + 1] = { -- adds a new element to the end of the table.
                ["x"] = spritex,
                ["y"] = spritey
            }
        end
...
function getExtendedSprites()
	-- [[ It reads from a separate table of sprite data for sprites such as offscreen enemies or objects.]]
    local extended = {}
    for slot = 0, 11 do
...
function getInputs()
    ...
    for dy = -BoxRadius * 16, BoxRadius * 16, 16 do
        for dx = -BoxRadius * 16, BoxRadius * 16, 16 do
            ...
            if tile == 1 and marioY + dy < 0x1B0 then -- if tile solid and above mario
                inputs[#inputs] = 1
            end
